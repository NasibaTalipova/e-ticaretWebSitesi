    ...
    <div class="row book-list">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo '<img src="' . $row["fotograf"] . '" class="card-img-top" alt="' . $row["UrunAdi"] . '">';
                echo '<div class="card-body">';
                echo '<h5 class="card-title">' . $row["UrunAdi"] . '</h5>';
                echo '<p class="card-text">' . $row["Aciklama"] . '</p>';
                echo '<p class="card-text">Fiyat: ' . $row["UrunFiyat"] . ' TL</p>';
                echo '<button class="btn btn-primary add-to-cart" data-id="' . $row["Urun_ID"] . '">Sepete Ekle</button>';
                echo '<button class="btn btn-secondary add-to-favorites" data-id="' . $row["Urun_ID"] . '">Favorilere Ekle</button>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
            }
    ...
    <div class="basket-modal">
      <div class="basket-items">
        ...
        <div class="basket-total">
          ...
          <button  class="btn-purple addToCartBtn">Sepete ekle</button>
        </div>
      </div>
    </div>
